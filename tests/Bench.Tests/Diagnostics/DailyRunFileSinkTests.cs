...
public sealed class DailyRunFileSinkTests
{
    ...
    public void A_run_writes_its_first_file_named_for_the_moment_it_started()
    {
        ...
        using var sink = new DailyRunFileSink(root, "bench-test", Template, At("2026-08-16T15:00:00"));

        Relative(root, sink.CurrentPath).Should().Be($"2026-08-16/bench-test-15-00-00-{Environment.ProcessId}.log");
    }
    ...
    public void A_run_that_outlives_the_day_continues_in_a_midnight_segment_under_the_next_days_folder()
    {
        ...
        Relative(root, sink.CurrentPath).Should().Be($"2026-08-17/bench-test-00-00-00-{Environment.ProcessId}.log");
    ...
    {
        var root = TempRoot();
        var sink = new DailyRunFileSink(root, "bench-test", Template, At("2026-08-16T15:00:00"));
        var first = sink.CurrentPath;
    ...
    private static string TempRoot() => Directory.CreateTempSubdirectory("bench-daily-log").FullName;

    private static string Relative(string root, string path) =>
        Path.GetRelativePath(Path.Combine(root, BenchLogging.LogFolder), path).Replace('\\', '/');
