...
public sealed class CellLifecycleTests
{
    ...
    {
        var claimed = CellLifecycle.Claim(Cell(), "worker-1", Noon).Ok();

        claimed.State.Should().Be(CellState.Claimed);
        claimed.Owner.Should().Be("worker-1");
    ...
    {
        CellLifecycle.Claim(Cell(), "  ", Noon).Reason()
    ...
    public void A_cell_cannot_be_claimed_twice()
    {
        ...
        CellLifecycle.Claim(claimed, "worker-2", Noon).Reason().Should().Contain("is Claimed, not Pending");
    }
    ...
    public void A_cell_that_has_used_up_its_attempts_is_abandoned_rather_than_requeued()
    {
        ...
        {
            cell = CellLifecycle.Reclaim(CellLifecycle.Claim(cell, $"host-{i}", Noon).Ok());
        }
    ...
    {
        var settled = CellLifecycle.Settle(CellLifecycle.Claim(Cell(), "w", Noon).Ok(), new LegOutcome.Completed()).Ok();
    ...
    {
        var claimed = CellLifecycle.Claim(Cell(), "w", Noon).Ok();
