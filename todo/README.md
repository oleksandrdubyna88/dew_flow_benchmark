...
| [PLAN_compute_backend_axis.md](PLAN_compute_backend_axis.md) | **plan only, 2026-08-16, blocked on a toolchain prerequisite** — the compute backend becomes an axis, and its first question is the operator's: on one R9700, is the sidecar faster under WSL (ROCm/MIGraphX) or on Windows (DirectML)? The question is answered three times, never once — steady-state throughput, cold start, and shape churn — because the flavours rank differently on each: MIGraphX compiles **per input shape** (2–4 min, ~2.5 GB apiece) while DirectML on this exact card held 30.6 GB of 32 and removed the D3D12 device three times in an hour. The instrument is the sidecar's own `PassTimings` split, whose compile field became trustworthy only on 2026-08-16 when it was scoped per engine. Structurally: two engines differing only in their sidecar are indistinguishable in a result row today (`engine = kind·endpoint·version·indexFingerprint`), so the engine must **echo** the backend it served on and a mismatch must block the cell — the same discipline as `PLAN_corpus_axis_integrity`. Phase 1 needs no code here and cannot run yet: the WSL arm wants ONNX Runtime 1.24.x built `--use_migraphx`, the same blocker as `dew_flow_sidecar_rust · research/PLAN_reliability_tail.md` item 1 |
