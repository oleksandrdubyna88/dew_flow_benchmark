...
| **Telemetry correlation** (agent MCP calls landing in `tool_telemetry` attributed to the leg) | **the tool benchmark**, with `dew_flow_mcp · todo/PLAN_tool_surface_config.md` as its other half | this plan consumes the attributed rows for the per-cell byte and count figures of §3.5 |
