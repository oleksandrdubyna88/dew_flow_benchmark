...
public sealed class DailyRunFileSink : ILogEventSink, IDisposable
{
    ...
    {
        root = Path.Combine(contentRoot, BenchLogging.LogFolder);
    ...
    /// path shape is spelled out; <see cref="BenchLogging.RunFilePath(string, string)"/> delegates here.
    /// </summary>
    public static string RunFilePath(string contentRoot, string appName, DateTimeOffset at) =>
        FilePath(
            Path.Combine(contentRoot, BenchLogging.LogFolder),
            appName,
